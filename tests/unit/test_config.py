...
def test_get_env_var_secret_key(monkeypatch) -> None:
...
def test_get_env_var_secret_key_raises_when_missing(monkeypatch) -> None:
    monkeypatch.delenv("SECRET_KEY", raising=False)
    with pytest.raises(ValueError, match="SECRET_KEY is not set"):
        get_env_var("SECRET_KEY")


def test_get_env_var_database_url(monkeypatch) -> None:
...
def test_get_env_var_database_url_raises_when_missing(monkeypatch) -> None:
